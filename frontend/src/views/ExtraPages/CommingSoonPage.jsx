...
const ComingSoon = memo(() => {
  ...
  useEffect(() => {
    function updateClock(clock, endtime) {
      ...
      if (clock) {
        clock.querySelector("[data-days]").textContent = Math.floor(
          t / 86400000
        );
        clock.querySelector("[data-hours]").textContent = format(
          Math.floor((t / 3600000) % 24)
        );
        clock.querySelector("[data-minutes]").textContent = format(
          Math.floor((t / 60000) % 60)
        );
        clock.querySelector("[data-seconds]").textContent = format(
                ...
                <div className="st-coming-inner text-white rounded-3">
                  ...
                  >
                    <li>
                      ...
                      <p className="days_text">{ t("streamCountdown.days") }</p>
                    </li>
                    <li>
                      <span className="hours" data-hours>
                        02
                      </span>
                      <p className="hours_text">{ t("streamCountdown.hours") }</p>
                    </li>
                    <li>
                      <span className="minutes" data-minutes>
                        18
                      </span>
                      <p className="minutes_text">{ t("streamCountdown.minutes") }</p>
                    </li>
                    <li>
                      <span className="seconds" data-seconds>
                        53
                      </span>
                      <p className="seconds_text">{ t("streamCountdown.seconds") }</p>
                    </li>
                  </ul>
                </div>
              </div>
